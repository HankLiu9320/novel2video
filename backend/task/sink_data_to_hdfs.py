...
ht = HiveTask()
WGET_CMD = "rsync -avPLq --bwlimit=100000 --append --timeout=600   {} {}/{}"
...
class WgetDataClass:
    ...
    def put_file_to_hdfs(self, local_path, local_file, hdfs_path, lzo):
        ...
        if lzo:
            hdfs_file_size_path = self.get_hdfs_file_size_and_path(local_file, hdfs_path, lzo)
            if int(hdfs_file_size_path[1]) >= 200000000:
                self.create_lzo_index(hdfs_path=hdfs_path, file_name=local_file)
    ...
    def wget_put_one_file(self, ip, data_day_str, data_day, data_hour, data_ten_min, server_file, server_final_file,
                          local_path, local_file, hdfs_path, continue_down=True, put_hdfs=True, lzo=True):
        """
            单文件下载
        :param ip: ip
        ...
        :return:
        """
        server_file = server_file.format(ip=ip, data_day_str=data_day_str, data_day=data_day, data_hour=data_hour, data_ten_min=data_ten_min)
        server_final_file = server_final_file.format(ip=ip, data_day_str=data_day_str, data_day=data_day, data_hour=data_hour, data_ten_min=data_ten_min)
        local_path = local_path.format(ip=ip, data_day_str=data_day_str, data_day=data_day, data_hour=data_hour, data_ten_min=data_ten_min)
        local_file = local_file.format(ip=ip, data_day_str=data_day_str, data_day=data_day, data_hour=data_hour, data_ten_min=data_ten_min)
        hdfs_path = hdfs_path.format(ip=ip, data_day_str=data_day_str, data_day=data_day, data_hour=data_hour, data_ten_min=data_ten_min)
        ...
        while True:
            ...
            try:
                if continue_down and current_date == data_day and current_hour == data_hour:
                    if current_min >= 58:
                        time.sleep(180)
                        continue
                    if self.check_remote_file_exists(server_file):
                        ...
                        ht.log.error(REMOTE_FILE_NOT_EXISTS.format(server_file))
                    time.sleep(60)
                else:
                    if self.check_remote_file_exists(server_final_file):
                        ...
                        if not self.check_wget_file(server_final_file, local_path + '/' + local_file):
                            ht.log.error('下载日志前后不一致')
                        ...
                        check_remote_count += 1
                        ht.log.error(REMOTE_FILE_NOT_EXISTS.format(server_final_file))
                        time.sleep(60)
                        if check_remote_count >= 6:
                            break
            except:
                pass

    def wget_put_files(self, ip_list, data_day_str, data_day, data_hour, data_ten_min, server_file, server_final_file,
                       local_path, local_file, hdfs_path, continue_down=True, put_hdfs=True, lzo=True ):
        process_list = []
        for ip in ip_list:
            process_list.append(multiprocessing.Process(target=self.wget_put_one_file,
                                                        args=(ip, data_day_str, data_day, data_hour, data_ten_min,
                                                              server_file, server_final_file, local_path, local_file,
                                                              hdfs_path, continue_down, put_hdfs, lzo)))
    ...
    def touch_success(self, hdfs_path, data_day_str, data_day, data_hour, all_job_count, job_index):
        ...
        if not self.is_hdfs_path_exists(hdfs_path):
            ...
            # 添加小时success文件
            #success_count_cmd = "hadoop fs -ls {}/ | grep '_success' | wc -l".format(hdfs_path)
            #success_count = self.run_shell_cmd(success_count_cmd)[1]
            #if int(success_count) == int(all_job_count):
            #    touch_hour_success_cmd = "hadoop fs -touchz {}/{}_success".format(hdfs_parent_path, data_hour)
            #    ht.log.info(touch_hour_success_cmd)
            #    try:
            #        self.run_shell_cmd(touch_hour_success_cmd)
            #    except:
            #        pass
    ...
    def rm_local_file_days_before(self, local_main_path, days):
        ...
        cmd1 = "find {path} -maxdepth 2 -mmin +480 -type f -exec /bin/rm -rf {{}} \\; ".format(path=local_main_path)
        cmd2 = "find {path} -maxdepth 1 -type d -empty -delete; ".format(path=local_main_path)
        ht.log.info(cmd1)
        ht.log.info(cmd2)
        try:
            self.run_shell_cmd_background(cmd1)
        except:
            pass
        try:
            self.run_shell_cmd_background(cmd2)
        except:
            pass
...
def main():
    ...
    if business not in cf.sections():
        ...
        raise '业务不正确'
    # ip_list = cf.get(business, 'ip_list')
    ...
    continue_down = cf.getboolean(business, 'continue_down')
    put_hdfs = cf.getboolean(business, 'put_hdfs')
    lzo = cf.getboolean(business, 'lzo')
    touch_success = cf.getboolean(business, 'touch_success')
    ...
    wget_data.wget_put_files(sub_ip_list, data_day_str, data_day, data_hour, data_ten_min, server_file,
                             server_final_file,local_path, local_file, hdfs_path, continue_down, put_hdfs, lzo)
    if touch_success:
        wget_data.touch_success(hdfs_path, data_day_str, data_day, data_hour, all_job_count, job_index)

    wget_data.rm_local_file_days_before(local_main_path, 0)
